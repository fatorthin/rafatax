...
class ListCostMou extends Page implements HasTable, HasForms, HasInfolists
{
    ...
    public function mount(int|string $record): void
    {
        ...
        ChecklistMou::query()->where('mou_id', '=', $this->mou->id, 'and')
            ...
            ->actions([
                \Filament\Tables\Actions\Action::make('editCost')
                    ...
                    ->action(function (array $data, CostListMou $record) {
                        ...
                        $this->cost_lists = CostListMou::query()->where('mou_id', '=', $this->mou->id, 'and')->get();
    ...
    public function form(Form $form): Form
    {
        return $form
            ->schema([
                Select::make('coa_id')
                    ->label('CoA')
                    ->options(Coa::query()->where('group_coa_id', '=', '40', 'and')->pluck('name', 'id'))
                    ->searchable()
                    ->placeholder('Pilih CoA'),
            ]);
    }

    private function getEditMouAction(): Actions\EditAction
    {
    ...
    private function getCreateCostAction(): Action
    {
        return Action::make('createCost')
            ->label('Tambah Biaya')
    ...
    private function getPreviewPdfAction(): Actions\Action
    {
    ...
    private function getExportPdfAction(): Action
    {
    ...
    private function getSendMouWhatsappAction(): Action
    {
    ...
    private function getCreateInvoiceAction(): Action
    {
        return Action::make('createInvoice')
            ...
            ->form([
                ...
                \Filament\Forms\Components\CheckboxList::make('checklist_mou_ids')
                    ...
                    ->options(function () {
                        return ChecklistMou::query()->where('mou_id', '=', $this->mou->id, 'and')
            ...
            ->action(function (array $data) {
                \Illuminate\Support\Facades\DB::transaction(function () use ($data) {
                    ...
                    if (!empty($checklistIds)) {
                        ...
                        ChecklistMou::query()->whereIn('id', $checklistIds, 'and', false)->update([
    ...
    private function getCreateOldInvoiceAction(): Action
    {
        return Action::make('createOldInvoice')
                    ...
                    ->schema([
                        \Filament\Forms\Components\Repeater::make('costListInvoices')
                            ->schema([
                                Select::make('coa_id')
                                    ->label('CoA')
                                    ->options(Coa::query()->where('group_coa_id', '=', '40', 'and')->orWhereIn('id', [162, 181])->pluck('name', 'id'))
                                    ->required()
                                    ->searchable()
                                    ->columnSpan([
                                        'md' => 3,
            ...
            ->action(function (array $data) {
                ...
                Notification::make()
                    ->title('Invoice lama berhasil ditambahkan')
                    ->success()
                    ->send();
    ...
    private function getBackAction(): Action
    {
    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            ActionGroup::make([
                ...
                $this->getCreateOldInvoiceAction(),
            ])
                ->label('Buat')
                ->icon('heroicon-o-plus')
                ->color('success')
                ->button(),
            ActionGroup::make([
                $this->getPreviewPdfAction(),
                $this->getExportPdfAction(),
                $this->getSendMouWhatsappAction(),
            ])
                ->label('Dokumen')
