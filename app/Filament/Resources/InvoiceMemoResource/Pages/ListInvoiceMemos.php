...
use App\Filament\Widgets\InvoiceMemoStats;
use Filament\Actions;
use Filament\Resources\Pages\ListRecords;

class ListInvoiceMemos extends ListRecords
{
    ...
    protected function getHeaderActions(): array
    {
        return [
            Actions\Action::make('export_excel')
                ...
                ->color('success')
                ->modalHeading('Export Data Invoice Memo')
                ...
                ->form([
                    ...
                    \Filament\Forms\Components\Select::make('invoice_type')
                        ->label('Tipe Invoice')
                        ->options(function () {
                            return \App\Models\Invoice::whereNotNull('memo_id')
                                ->select('invoice_type')
                ...
                ->action(function (array $data) {
                    $query = \App\Models\Invoice::with(['memo', 'costListInvoices'])
                        ->whereNotNull('memo_id');
                    ...
                    return response()->streamDownload(function () use ($invoices) {
                        ...
                        $sheet->setTitle('Data Invoice Memo');

                        $headers = ['No', 'No. Invoice', 'Tanggal Invoice', 'Klien', 'No. Memo', 'Status', 'Tipe', 'Total Tagihan', 'Dikirim'];
                        $col = 'A';
                        ...
                        foreach ($invoices as $index => $invoice) {
                            ...
                            $memoNumber = $invoice->memo ? $invoice->memo->no_memo : '-';
                            ...
                            $sheet->setCellValue('E' . $row, $memoNumber);
                    ...
                    }, 'Export_Invoice_Memo_' . date('Y-m-d_H-i-s') . '.xlsx');
                }),
            Actions\Action::make('export_rekap_piutang_bulanan')
                ->label('Export Rekap Piutang')
                ->icon('heroicon-o-document-chart-bar')
                ->color('warning')
                ->modalHeading('Export Rekap Piutang Bulanan Memo')
                ->modalSubmitActionLabel('Export')
                ->form([
                    \Filament\Forms\Components\Select::make('year')
                        ->label('Tahun')
                ...
                ->action(function (array $data) {
                    ...
                    return response()->streamDownload(function () use ($grouped, $monthNames, $data) {
                        $spreadsheet = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
                        $sheet = $spreadsheet->getActiveSheet();
                        $sheet->setTitle('Rekap Piutang Memo');

                        $yearTitle = $data['year'] ?? 'Semua Tahun';
                        $sheet->setCellValue('A1', 'Rekap Piutang Bulanan Memo - Tahun ' . $yearTitle);
                        ...
                        $writer = new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($spreadsheet);
                        $writer->save('php://output');
                    }, 'Rekap_Piutang_Bulanan_Memo_' . date('Y-m-d_H-i-s') . '.xlsx');
                }),
