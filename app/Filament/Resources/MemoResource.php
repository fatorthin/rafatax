            ...
            ->columns([
                Tables\Columns\TextColumn::make('no_memo')
                    ...
                    ->extraHeaderAttributes([
                        'style' => 'position: sticky; left: 0; z-index: 30; background-color: inherit; box-shadow: inset -2px 0 4px -2px rgba(0,0,0,0.1);',
                ...
                Tables\Columns\TextColumn::make('selisih')
                    ...
                    ->numeric(locale: 'id')
                    ->getStateUsing(function ($record) {
                        $totalInvoice = \App\Models\CostListInvoice::whereHas('invoice', function ($query) use ($record) {
                            $query->where('memo_id', $record->id);
                        })->sum('amount');
